...
def main():
    ...
    sql_create_employees_study_table = """ CREATE TABLE IF NOT EXISTS employees_study_dop (
...
class Employees_study_dop:
    ...
    def __init__(self, window):
        ...
        ttk.Button(frame, text='Сохранить', command=self.add_employees_study_dop).grid(row=4, columnspan=3, sticky=W + E)
        ...
        ttk.Button(frame, text='Удалить', command=self.delete_employees_study_dop).grid(row=6, column=0, sticky=W + E)
        ttk.Button(frame, text='Изменить', command=self.edit_word).grid(row=6, column=1, sticky=W + E)

        # заполнение таблицы
        self.get_employees_study_dop()
    ...
    def get_employees_study_dop(self):
        records = self.tree.get_children()
        for element in records:
            self.tree.delete(element)
        query = 'SELECT em.name, ts.name FROM employees_study_dop es join employees1 em on es.id_employees = em.id join type_of_study ts on es.id_type_study = ts.id ORDER BY em.name DESC'
    ...
    def add_employees_study_dop(self):
        if self.validation():
            query = 'INSERT INTO employees_study_dop SELECT em.id as id_employees, ts.id as id_type_study from employees1 em cross join type_of_study ts where em.name  = ? and ts.name = ?'
    ...
    # удаление слова
    def delete_employees_study_dop(self):
        self.message['text'] = ''
        try:
...
if __name__ == '__main__':
    window = Tk()
    application = Employees_study_dop(window)
    window.mainloop()